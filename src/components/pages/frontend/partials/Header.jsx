    ...
    >
      <div className="container">
        <div
          ...
          } flex justify-between items-center py-4 text-white md:hover:text-black`}
        >
         <button
            className={`${isOpen ? "active" : ""} menu-btn md:hidden `}
            onClick={() => setIsOpen(!isOpen)}
          >
            <span className={`${scrollPosition === 0 ? "bg-white" : "bg-black"} ${
                isOpen && "!bg-black"
              }`}
            ></span>
            <span
          ...
          <Link to="/" className={`${isOpen ? "!text-black" : ""} text-xl uppercase font-bold`}>
            Zanerobe
          </Link>
          ...
          <ul className={`${isOpen ? "!text-black" : ""} flex gap-5  group-hover:text-black`}>
